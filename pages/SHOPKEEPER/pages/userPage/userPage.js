// pages/SHOPKEEPER//pages/userPage/userPage.js
var app = getApp();
...
Page({
  ...
  data: {
    ...
    btn_index: 0,//Tab的索引0：上架作品，1：下架作品
    isOver: false,//底部提示信息,
  ...
  grassFun: function(page){
    ...
    let data = {
      ...
      cfrom: 1,
      anchor_id: that.data.otheruid,
      page: page,
      pagesize:10
    };
    if(that.data.btn_index == 0){//上架
      data.status = 1
    }else if(that.data.btn_index == 1){//下架
      data.status = 3
    }
    common.post(url, data, function callBack(res) {
      if (res.err_code == 0) {
        that.setData({
          isSelf: res.err_msg.userinfo.ismyself*1//1:是本人,0:不是本人
        });
        if(page == 1){
          that.setData({
  ...
  followClick:function(e){
    let that = this; 
    let subscribe = e.currentTarget.dataset.subscribe;
    let url = 'app.php?c=society&a=subscribe',
      data = {
        store_id: app.globalData.store_id || common.store_id,
        uid: that.data.grassData.userinfo.uid
      };
      if(subscribe == 1){//已关注，点击取消关注
        data.status = 0
      }else if(subscribe == 0){//未关注，点击关注
        data.status = 1
      }
    common.post(url, data, function(){
      if(subscribe == 1){//已关注，点击取消关注
        ...
        that.setData({
          'grassData.userinfo.is_subscribe': 0
        });
      }else if(subscribe == 0){//未关注，点击关注
        publicFun.warning('关注成功', that);
        that.setData({
          'grassData.userinfo.is_subscribe': 1
        });
      }
    }, '', '', true)
  },
  ...
  menuClick:function(e){
    ...
    }else if(menutype == 'downshelf' || menutype == 'upshelf'){//下架/上架
      let url = 'app.php?c=society&a=change_status',
        data = {
          store_id: app.globalData.store_id || common.store_id,
          society_id: society_id
        };
      if(menutype == 'downshelf'){//下架
        data.status = 3;
      }else if(menutype == 'upshelf'){//上架
        data.status = 1
      }
      common.post(url, data, function(){
        ...
        if(menutype == 'downshelf'){//下架
          ...
          that.setData({
            'grassData.userinfo.onshelfcount': --grassData.userinfo.onshelfcount,
            'grassData.userinfo.offshelfcount': ++grassData.userinfo.offshelfcount,
            'grassData.list': grassData.list
          });
        }else if(menutype == 'upshelf'){//上架
          publicFun.warning('上架成功', that);
          that.setData({
            'grassData.userinfo.onshelfcount': ++grassData.userinfo.onshelfcount,
            'grassData.userinfo.offshelfcount': --grassData.userinfo.offshelfcount,
            'grassData.list': grassData.list
  ...
  creatPost: function () {
    ...
    let canvasData = { // 画布数据
      ...
      shopName: that.data.grassData.userinfo.store_name,
