...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.teamstyle == 2){//2：团队结算，1：邀请付费
      that.setData({
        isTeam: true,
        teamstyle:options.teamstyle
      })
    } else if (options.teamstyle == 1){//邀请付费
      that.setData({
        isTeam: false,
        teamstyle:options.teamstyle
      });
      if(options.icoed){//分享进入
        that.setData({
          "group_detail.icoed": options.icoed,
          haveCode: true
        });
      }
    }
    ...
    if (options.scene != undefined && options.scene != 'wxapp') {
      ...
      console.log("直播注册二维码", scene_id);
      if (scene_id) {
        // var scene_arr = scene.split(',');
        // app.globalData.store_id = scene_arr[0];
        // app.globalData.share_uid = scene_arr[2];
    ...
    if (getApp().globalData.anchor_team_type){
      ...
      let anchorTeamType = getApp().globalData.anchor_team_type*1
      if(anchorTeamType == 1){//邀请付费
        that.setData({
          isTeam: false
        });
      }else if(anchorTeamType == 2){//2：团队结算
        that.setData({
          isTeam: true
        })
      }
  ...
  pageFun: function(result) {
    let that = this;
    let url = 'app.php?c=team&a=get_page_info';
    console.log(app.globalData.share_uid,'share_uid')
    let data = {
        store_id: app.globalData.store_id || common.store_id,
        share_uid: app.globalData.share_uid || 8367
      };
  ...
  grouperSave:function(){
    ...
    if (!that.data.isTeam) {//邀请付费
      params.apply_rule_level = that.data.gradeData[that.data.isGrade].level//申请角色
      params.is_pay = that.data.pageData.open_pay//是否支付
      params.invite_store_id = that.data.fxId//邀请店铺参数
    }else if(that.data.isTeam){//团队模式
      params.team_id = that.data.pageData.team_id
      params.team_level = that.data.pageData.team_level
    }
    var url;
    if (that.data.isTeam){//团队结算
      url = "app.php?c=team&a=apply_team"
    } else {//邀请付费
      url = "app.php?c=drp&a=save_agent"
    }
    common.post(url, params, function (res) {
      if (res.err_code == 0) {
        that.setData({
  ...
  getTelcode: function () {
    ...
    if (!phone_no) {
      return publicFun.warning('请输入手机号', that);
    }
    ...
    common.post(url, datas, function (res) {
      ...
      console.log('验证码', res)
      return publicFun.warning('发送成功', that);
    }, '')
  },
  // 上传身份证
  upImg:function(e){
